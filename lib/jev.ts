...
export const JEV_CONFIDENCE_THRESHOLD = 0.3;
...
export async function discoverTables(
  ...
  let cursor = 0;
  // One request per table, with bounded concurrency and no automatic retries.
  await Promise.all(
    Array.from({ length: Math.min(CONCURRENCY, tables.length) }, async () => {
    ...
    // Stop queued and in-flight work when any table fails.
    sweep.abort(error);
    throw error;
  });
