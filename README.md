...
After applying the review, **Automatic labeling** uses the selected OpenRouter writing model and the connection's existing API key. Four tables are investigated concurrently. For each table, JEV evaluates full linked functions from the saved code archive for row meaning, lifecycle, business purpose and relationships. A diverse selection of code is shown to the writer alongside the actual schema. The writer can inspect specific linked functions, request one additional targeted JEV search, inspect related active schemas, and attempt up to three SELECT queries. A mandatory second JEV sweep checks its draft and explicit claims for contradictions, omissions and unresolved questions before the writer finalizes a short Spanish description (maximum 240 characters).
...
Each table has at most eight writer calls, three JEV sweeps and three minutes; the overall batch has a thirty-minute deadline. JEV requests run eight at a time per active table. Code sent to the writer is limited to 120 KB, and individual functions over 100 KB are skipped by JEV. Missing code, failed JEV decisions and incomplete evidence are recorded as warnings; JEV scores are selection signals, not verified facts. A bounded, connection-scoped in-memory cache reuses decisions for identical code, schema and questions; changed inputs are reevaluated. Query results are never cached.
