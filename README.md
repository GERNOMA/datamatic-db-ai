...
In **Modelos, Controladores y Más → Archivo guardado en la web**, upload a `.rar` or `.zip` containing the exported `con-codigo/` JSONs. RAR decoding uses [node-unrar-js](https://github.com/YuJianrong/node-unrar.js). Only JSONs whose table exists in the connected database and their validated functions are saved per database connection, as a filtered ZIP in `.datamatic/code/` on the server. Keep this directory on persistent storage when deploying; it is excluded from Git and browser workspace exports. Uploading raw PHP here is not supported: analyze it first using the existing project analyzer. The summary shows database tables, saved tables, distinct tables in the original JSONs, and the original JSON file count. Models without a matching table are omitted, including their links on shared functions. Existing saves are filtered on access. A failed upload leaves the previous archive intact.
